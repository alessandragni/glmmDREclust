...
def algorithm_t(ran_var, ran_int, n_fix, sim, tol, model, fix, lengths, y, N, t):
    ...
    while (conv1 == 0 or conv2 == 0) and (no_group == 0) and (k < AC.K):
        ...
        while ((sum(sum(AC.tol > D)) != AC.nknots * AC.nknots) and (sum(sum(AC.tol > D)) > AC.nknots) and (
                no_group == 0)):
            # two close masses are collapsing
            knots_new = AC.knots
            weights_new = AC.weights

            minimum = np.min(D[np.nonzero(D)])
            (row, col) = np.where(D == minimum)[0]
            # print('collapsing')
            # print((row, col))
            ...
            if AC.nknots == 1:
                # print("there are no different groups")
                no_group = 1
                break
